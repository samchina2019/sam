...
- (void)showCalendarDate:(NSDate *)date {
    ...
    if (date) { // 初始化 今日
        ...
        NSString *distanceTime = [NSString stringWithFormat:@"%ld-%ld-%ld", [arraySystem[0] integerValue], [arraySystem[1] integerValue], [arraySystem[2] integerValue]];
    ...
    if (self.returnTitleValueBlock) {
        self.returnTitleValueBlock([NSString stringWithFormat:@"%ld-%ld", (long) _year, (long) _month]);
    }
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return CGSizeMake((ViewWidth - 60) / 7 - 2, (ViewWidth - 60) / 7);
}
...
//    cell.numberBtn.hidden=YES;
    ...
    if (indexPath.row < dayInWeek || (indexPath.row) % 7 == 0 || (indexPath.row) % 7 == 6) {
        ...
        if (indexPath.row < dayInWeek || indexPath.row >= (_monthLength + dayInWeek)) {
            ...
            cell.calendarTitleColor =  UIColorFromRGB(0xFC5458);
            if (self.titleArray.count > 0) {
                NSInteger insex = [self.dayArray[indexPath.row] intValue] - 1;
                NSDictionary *dict = self.titleArray[insex];
                if ((insex + 1) == [dict[@"day"] intValue]) {
                    if (self.isComeBaobiao ==YES ) {
                        if ([dict[@"status"] intValue]==1) {
                            if ([dict[@"num"] intValue]==0) {
                                 cell.numberBtn.hidden=YES;
                            }else{
                            [cell.numberBtn setTitle:[NSString stringWithFormat:@"%@",dict[@"num"]] forState:UIControlStateNormal];
                            [cell.numberBtn setTitleColor:UIColorFromRGB(0x3FAEE9) forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                            }
                        }else{
                            if ([dict[@"num"] intValue]==0) {
                                cell.numberBtn.hidden=YES;
                            ...
                            [cell.numberBtn setTitle:[NSString stringWithFormat:@"%@",dict[@"num"]] forState:UIControlStateNormal];
                            [cell.numberBtn setTitleColor:UIColorFromRGB(0xFA5458) forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                            }
                        }
                    }else{
                        if ([dict[@"status"] intValue] == 6) {
                            [cell.numberBtn setTitleColor:[UIColor clearColor] forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@"duihaolan"] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                        } else if ([dict[@"status"] intValue] == 5) {
                            
                            [cell.numberBtn setTitleColor:[UIColor clearColor] forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@"duihao"] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                        } else if ([dict[@"status"] intValue] == 4) {
                            [cell.numberBtn setTitle:@"迟到" forState:UIControlStateNormal];
                            [cell.numberBtn setTitleColor:UIColorFromRGB(0xFA5458) forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                        } else if ([dict[@"status"] intValue] == 3) {
                            [cell.numberBtn setTitle:@"异常" forState:UIControlStateNormal];
                            [cell.numberBtn setTitleColor:UIColorFromRGB(0xFA5458) forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                            
                        } else if ([dict[@"status"] intValue] == 0) {
                            [cell.numberBtn setTitle:@"未" forState:UIControlStateNormal];
                            [cell.numberBtn setTitleColor:UIColorFromRGB(0xFA5458) forState:UIControlStateNormal];
                            [cell.numberBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
                            cell.numberBtn.hidden=NO;
                            
                        } else if ([dict[@"status"] intValue] == -1) {
    ...
    if (todayBool == YES) {
        if (indexPath.row == (_systemDay + dayInWeek) - 1) {
            ...
            cell.calendarTitleColor = [UIColor colorWithRed:51 / 255.0 green:51 / 255.0 blue:51 / 255.0 alpha:1.0];
    ...
    if (_selectedRow == indexPath.row) {
        ...
        cell.backgroundColor = UIColorFromRGB(0x3FAEE9);
    }
...
- (void)selectDate:(NSString *)dayDate showStatus:(NSString *)status {
    ...
    if (diff.day > 0) { // 多少日后
        ...
        if (diff.year > 0) {
            distanceTime = [NSString stringWithFormat:@"%ld-%ld-%ld", [arraySystem[0] integerValue], [arraySystem[1] integerValue], [arraySystem[2] integerValue]];
        }
        if (diff.year == 0) {
            distanceTime = [NSString stringWithFormat:@"%ld-%ld-%ld", [arraySystem[0] integerValue], [arraySystem[1] integerValue], [arraySystem[2] integerValue]];
        }
...
- (NSInteger)calculationThisMonthDays:(NSDate *)days {
    NSCalendar *calendar = [NSCalendar currentCalendar];
    if (days == nil || [days isKindOfClass:[NSNull class]]) {
        days = [NSDate date];
    }
