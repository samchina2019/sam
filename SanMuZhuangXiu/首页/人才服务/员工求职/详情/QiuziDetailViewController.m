//
//  QiuziDetailViewController.m
//  SanMuZhuangXiu
//
//  Created by 王巧云 on 2019/4/24.
...
@interface QiuziDetailViewController ()<KNPhotoBrowerDelegate, MBProgressHUDDelegate,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource>
...
@implementation QiuziDetailViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)initCollectionView {
    ...
    CGFloat width = (ViewWidth - 32-16)/3;
...
- (void)getJianliDetailData {
    [DZNetworkingTool postWithUrl:kQiuziJobInfo
      ...
      if ([responseObject[@"code"] intValue] == SUCCESS) {
          ...
          if ([self.dataDict[@"salary"] length] ==0) {
               self.xinziLabel.text = [NSString stringWithFormat:@"0元/月"];
          }else{
          self.xinziLabel.text = [NSString stringWithFormat:@"%@",self.dataDict[@"salary"]];
          }
          self.gongzhongLabel.text = self.dataDict[@"recruitment_post"];
          ...
          if (self.array.count>0) {
              ...
              self.scrollViewHeight.constant = ceil(((self.array.count - 1) / 3 + 1) * (ViewWidth - 32-16) / 3.0 + 5 * floorf((self.array.count - 1) / 3)) + 470;
          }
