...
//            [self alertPinpaiViewWithcellView:weakself];
        ...
        cell.yixuanBlock = ^{
            ...
            self.selectSpecView.addpinpaiBlock = ^ {
                
             [self alertPinpaiViewWithcellView:_selectSpecView];
            ...
            self.selectSpecView.tebleViewHeight.constant = self.guigeArray.count * 50+SCREEN_WIDTH/750*210;
...
- (void)alertPinpaiViewWithcellView:(SelectSpecView *)view {
    ...
    for (int i = 0; i < self.pinpaiArray.count; i++) {
        ...
        [alert addAction:[UIAlertAction actionWithTitle:typeModel.name
                                                ...
                                                handler:^(UIAlertAction *_Nonnull action) {
                                                    [self alertXinziClick:i intoCell:view];
                                                }]];
    }
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"取消"
                                            ...
                                            handler:^(UIAlertAction *_Nonnull action) {
                                                [self alertXinziClick:self.pinpaiArray.count intoCell:view];
                                            }]];
    //弹出提示框； _xuanzefild
    [self presentViewController:alert animated:true completion:nil];
}

- (void)alertXinziClick:(NSInteger)rowInteger intoCell:(SelectSpecView *)view;
{
    view.xuanzefild.text = @"";
    if (rowInteger < self.pinpaiArray.count) {

        PinpaiModel *typeModel = self.pinpaiArray[rowInteger];

        view.xuanzefild.text = typeModel.name;
...
//        self.totalLabel.hidden = YES;
        ...
        [ self.cartBtn  setBackgroundImage:[UIImage imageNamed:@"icon_cailiaodan"] forState:UIControlStateNormal];
...
//        self.totalLabel.hidden = NO;
        ...
        [ self.cartBtn  setBackgroundImage:[UIImage imageNamed:@"icon_cailiaodan_yixuan"] forState:UIControlStateNormal];
