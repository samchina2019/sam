//
//  GoodsBrowseHistoryListViewController.m
//  SanMuZhuangXiu
//
...
@interface GoodsBrowseHistoryListViewController ()
...
@implementation GoodsBrowseHistoryListViewController
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"GoodsFocusListCell" bundle:nil] forCellReuseIdentifier:@"GoodsFocusListCell"];
    [self.tableView.mj_header beginRefreshing];
    //注册通知(接收,监听,一个通知)
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteClick:) name:@"dianpuchu" object:nil];
}
...
//    kGoodsVisitList
        ...
        [DZNetworkingTool postWithUrl:kGoodsVisitList params:params success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            if ([responseObject[@"code"] intValue] == SUCCESS) {
                ...
                for (NSDictionary * temp in array) {
                    GoodsListModel * model = [GoodsListModel mj_objectWithKeyValues:temp];
                    model.zujiId = [temp[@"id"] intValue];
                    [self.dataArray addObject:model];
                }
                
                [self.tableView reloadData];
                if (self.dataArray.count == [dict[@"num"] intValue]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GoodsFocusListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GoodsFocusListCell" forIndexPath:indexPath];
    GoodsListModel  *model = self.dataArray[indexPath.row];
    cell.nameLabel.text = [NSString stringWithFormat:@"%@",model.goods_name];
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:model.goods_image] placeholderImage:[UIImage imageNamed:@"defaultImg"]];
    
    cell.storeNameLabel.text = [NSString stringWithFormat:@"%@",model.store_name];
...
- (IBAction)selectBtnClick:(UIButton *)sender {
    if ([[sender titleForState:UIControlStateNormal] isEqualToString:@"删除"]) {
        ...
        NSMutableArray *array=[NSMutableArray array];
                for (GoodsListModel *model  in self.dataArray) {
        ...
        if (self.dataArray.count == 0) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"dianpuData" object:nil userInfo:dict];
