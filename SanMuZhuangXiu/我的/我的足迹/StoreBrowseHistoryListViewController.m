//
//  StoreBrowseHistoryListViewController.m
//  SanMuZhuangXiu
//
...
@interface StoreBrowseHistoryListViewController ()
...
@implementation StoreBrowseHistoryListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.navigationItem.title = @"商家浏览历史";
    ...
    //注册通知(接收,监听,一个通知)
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteClick:) name:@"shangpinchu" object:nil];
}
...
- (void)getDataArrayFromServerIsRefresh:(BOOL)isRefresh
{
    NSDictionary *params = @{@"page":@(_page),
                             @"row":@(10)};
    [DZNetworkingTool postWithUrl:kStoreZuJiList params:params success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"code"] intValue] == SUCCESS) {
            ...
            for (NSDictionary *dict in array) {
                ZuJiListModel *model = [ZuJiListModel mj_objectWithKeyValues:dict];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZuJiListModel *model = self.dataArray[indexPath.row];
    cell.starView.actualScore = model.level;
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:model.store_avatar] placeholderImage:[UIImage imageNamed:@"defaultImg"]];
    cell.storeNameLabel.text = model.seller_name;
    if(model.distance<0 ){
    cell.distanceLabel.text = [NSString stringWithFormat:@"暂未获取到经纬度"];
    }else{
    cell.distanceLabel.text = [NSString stringWithFormat:@"%.2fkm",model.distance/1000];
    }
    cell.yimaiLabel.text = [NSString stringWithFormat:@"%d人已买",model.order_count];
    cell.goodsLabel.text = [NSString stringWithFormat:@"主营商品：%@",model.seller_type_name];
    if (model.man.length == 0) {
    cell.huodongLabel.text = [NSString stringWithFormat:@"暂无数据"];
    }else{
    cell.huodongLabel.text = [NSString stringWithFormat:@"满%@减%@",model.man,model.subtract];
    }
    cell.peisongLabel.text = [NSString stringWithFormat:@"配送费:%@",@"0"];
    cell.block = ^{
        ...
        [alertC addAction:[UIAlertAction actionWithTitle:@"是"
                                                 ...
                                                 handler:^(UIAlertAction *_Nonnull action) {
                                                     //删除
                                                     NSString *zujiId=[NSString stringWithFormat:@"%d",model.zuji_id];
...
-(void)deleteDataWithId:(NSString *) zujiId{
    ...
    [DZNetworkingTool postWithUrl:kDeleteVisit params:dict success:^(NSURLSessionDataTask *task, id responseObject) {
...
//        NSString *zujiId=[self.dataArray componentsJoinedByString:@","];
        ...
        for (ZuJiListModel *model  in self.dataArray) {
            [array addObject:@(model.zuji_id)];
        }
       NSString *zujiId = [array componentsJoinedByString:@","];
        ...
        if (self.dataArray.count == 0) {
             ...
             [[NSNotificationCenter defaultCenter] postNotificationName:@"shanpinData" object:nil userInfo:dict];
