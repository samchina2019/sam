//
//  ServerFocusListViewController.m
//  SanMuZhuangXiu
//
...
#import "ServerFocusListZhaoPinCell.h"
#import "GuanzhuListModel.h"

@interface ServerFocusListViewController ()
...
@implementation ServerFocusListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.navigationItem.title = @"服务关注";
    self.type=@"30";
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"ServerFocusListCell" bundle:nil] forCellReuseIdentifier:@"ServerFocusListCell"];
    [self.tableView registerNib:[UINib nibWithNibName:@"ServerFocusListZhaoPinCell" bundle:nil] forCellReuseIdentifier:@"ServerFocusListZhaoPinCell"];
...
- (void)getDataArrayFromServerIsRefresh:(BOOL)isRefresh
{
    ...
    [DZNetworkingTool postWithUrl:kMyFollowServer params:params success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model.service_type isEqualToString:@"企业招聘"]) {
        ServerFocusListZhaoPinCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ServerFocusListZhaoPinCell" forIndexPath:indexPath];
        GuanzhuListModel *model=self.dataArray[indexPath.row];
    cell.titleLabel.text=[NSString stringWithFormat:@"%@",model.name];
//    cell.companyNameLabel.text=[NSString stringWithFormat:@"%@",model.];
    
#warning 信息修改
        return cell;
    }else{
        ServerFocusListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ServerFocusListCell" forIndexPath:indexPath];
        cell.titleLabel.text=[NSString stringWithFormat:@"%@",model.name];
        cell.typeLabel.text=[NSString stringWithFormat:@"%@",model.address];
        // 时间戳 -> NSDate *
